...
public class ListMzAdapter extends BaseListAdapter<HomeDataModel.GoodlistsBean>
{

    public ListMzAdapter(Context context, List<HomeDataModel.GoodlistsBean> list)
    {
        super(context, list);
    }
    ...
    {
        if (convertView==null)
            convertView=mInflater.inflate(R.layout.item_home_product_minus,null);
        ...
        TextView tv_levelnote = ViewHolder.get(convertView,R.id.tv_levelnote);

        convertView.setLayoutParams(new ViewGroup.LayoutParams(DensityUtils.getScreenWidth(),ViewGroup.LayoutParams.WRAP_CONTENT));
        ...
        tv_levelnote.setVisibility(TextUtils.isEmpty(itemBean.levelnote)?View.INVISIBLE:View.VISIBLE);
        tv_levelnote.setText(itemBean.levelnote);
        Bitmap b=null;
        L.i(itemBean.type);
        if (TextUtils.equals(itemBean.type,"3"))
        {
